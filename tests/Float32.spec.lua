...
return function()
	...
	describe("De/Serialization", function()
		local function getEpsilon(value: number): number
			return 1 / 2^(23 - math.log(value, 2))
		end
		...
		it("Max safe value", function()
			local buffer = BitBuffer.new()
			buffer:WriteFloat32(2^23 - 1)
			buffer._index = 0
			expect(buffer:ReadFloat32()).to.be.equal(2^23 - 1)
		end)

		it("Min safe value", function()
			local buffer = BitBuffer.new()
			buffer:WriteFloat32(-1 * (2^23 - 1))
			buffer._index = 0
			expect(buffer:ReadFloat32()).to.be.equal(-1 * (2^23 - 1))
		end)
