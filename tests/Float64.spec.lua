...
return function()
	...
	describe("De/Serialization", function()
		local function getEpsilon(value: number): number
			return 1 / 2^(53 - math.log(value, 2))
		end
		...
		it("Max safe value", function()
			local buffer = BitBuffer.new()
			buffer:WriteFloat64(2^23 - 1)
			buffer._index = 0
			expect(buffer:ReadFloat64()).to.be.equal(2^23 - 1)
		end)

		it("Min safe value", function()
			local buffer = BitBuffer.new()
			buffer:WriteFloat64(-1 * (2^23 - 1))
			buffer._index = 0
			expect(buffer:ReadFloat64()).to.be.equal(-1 * (2^23 - 1))
		end)
